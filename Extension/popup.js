...
function handleSwitchStatusAndStorage(state, status, key) {
  if (state) {
    // STATE IS EITHER TRUE (THIS SETTING GOT ENABLED) OR FALSE (THIS SETTING GOT DISABLED)
    status.innerHTML = "ON";
...
function isAlreadyInList(text, key) {
  ...
  for (let i = 0; i < ieList.children.length; ++i) {
    for (let j = 0; j < ieList.children[i].childNodes.length; ++j) {
      if (
        ieList.children[i].childNodes[j].nodeType == Node.TEXT_NODE &&
        text == ieList.children[i].childNodes[j].textContent
      ) {
        return true;
      }
    }
...
function addIeElement(text, key, isDocumentInitializing) {
  ...
  if (key == "videoExclusionsList") {
    ieList.appendChild(
      createListHtml(text, VIDEO_EXCLUSION_KEY, VIDEO_EXCLUSION_REMOVE_KEY),
    );
  } else if (key == "keywordExclusionsList") {
    ieList.appendChild(
      createListHtml(text, KEYWORD_EXCLUSION_KEY, KEYWORD_EXCLUSION_REMOVE_KEY),
    );
  } else if (key == "videoInclusionsList") {
    ieList.appendChild(
      createListHtml(text, VIDEO_INCLUSION_KEY, VIDEO_INCLUSION_REMOVE_KEY),
    );
  } else if (key == "keywordInclusionsList") {
    ieList.appendChild(
      createListHtml(text, KEYWORD_INCLUSION_KEY, KEYWORD_INCLUSION_REMOVE_KEY),
    );
  }
...
function initializeDocument(tab) {
  ...
  chrome.runtime.getPlatformInfo(function (info) {
    if (info.os != "win") {
      document.getElementById("osIncompatibleContainer").style.display =
        "block";
    } else {
      // NODE CONNECTION ERROR
      chrome.storage.sync.get("isNativeConnected", function (result) {
        if (result.isNativeConnected == false) {
          document.getElementById("clientErrorContainer").style.display =
            "block";
        } else {
          ...
          chrome.storage.sync.get("nativeVersionStatus", function (result) {
            if (result.nativeVersionStatus == -1) {
              let versionWarningContainer = document.getElementById(
                "versionWarningContainer",
              );
              let versionWarningMinimizeBody =
                versionWarningContainer.querySelector(".minimizeBody");
              let versionWarningReturnBack =
                versionWarningContainer.querySelector("span.returnBack");

              versionWarningContainer.style.display = "block";
              chrome.storage.sync.get(
                "minimizeVersionWarning",
                function (result) {
  ...
  chrome.storage.sync.get("enableOnStartup", function (result) {
    ...
    let statusSwitch = enableOnStartupLabel.querySelector(
      "label.switch > input",
    );
  ...
  if (
    tab.url.startsWith(YOUTUBE_MAIN_URL) ||
    tab.url.startsWith(YOUTUBE_MUSIC_URL)
  ) {
    chrome.storage.sync.get("tabEnabledList", function (result) {
      let status = enableOnThisTabLabel.querySelector("span.switchStatus");
      let statusSwitch = enableOnThisTabLabel.querySelector(
        "label.switch > input",
      );
    ...
    // REMOVE ABILITY TO INTERACT WITH [ENABLE ON THIS TAB] IF TAB IS NOT YOUTUBE OR YOUTUBE MUSIC
    let status = enableOnThisTabLabel.querySelector("span.switchStatus");
  ...
  chrome.storage.sync.get("enableExclusions", function (result) {
    ...
    let statusSwitch = enableExclusionsLabel.querySelector(
      "label.switch > input",
    );
  ...
  chrome.storage.sync.get("enableInclusions", function (result) {
    ...
    let statusSwitch = enableInclusionsLabel.querySelector(
      "label.switch > input",
    );
  ...
  chrome.storage.sync.get("keywordExclusionsList", function (result) {
    for (let i = 0; i < result.keywordExclusionsList.length; ++i) {
      addIeElement(
        result.keywordExclusionsList[i],
  ...
  chrome.storage.sync.get("keywordInclusionsList", function (result) {
    for (let i = 0; i < result.keywordInclusionsList.length; ++i) {
      addIeElement(
        result.keywordInclusionsList[i],
        "keywordInclusionsList",
        true,
      );
    }
  });
  ...
  let enableYouTubeMusicLabel = document.getElementById(
    "enableYouTubeMusicLabel",
  );
  chrome.storage.sync.get("enableYouTubeMusic", function (result) {
    ...
    let statusSwitch = enableYouTubeMusicLabel.querySelector(
      "label.switch > input",
    );
  ...
  let enableVideoButtonLabel = document.getElementById(
    "enableVideoButtonLabel",
  );
  chrome.storage.sync.get("enableVideoButton", function (result) {
    ...
    let statusSwitch = enableVideoButtonLabel.querySelector(
      "label.switch > input",
    );
  ...
  let enableChannelButtonLabel = document.getElementById(
    "enableChannelButtonLabel",
  );
  chrome.storage.sync.get("enableChannelButton", function (result) {
    ...
    let statusSwitch = enableChannelButtonLabel.querySelector(
      "label.switch > input",
    );
  ...
  let enablePlayingIconLabel = document.getElementById(
    "enablePlayingIconLabel",
  );
  chrome.storage.sync.get("enablePlayingIcon", function (result) {
    ...
    let statusSwitch = enablePlayingIconLabel.querySelector(
      "label.switch > input",
    );
  ...
  let useAlbumThumbnailLabel = document.getElementById(
    "useAlbumThumbnailLabel",
  );
  chrome.storage.sync.get("useAlbumThumbnail", function (result) {
    ...
    let statusSwitch = useAlbumThumbnailLabel.querySelector(
      "label.switch > input",
    );
  ...
  chrome.storage.sync.get("useThumbnailIcon", function (result) {
    ...
    let statusSwitch = useThumbnailIconLabel.querySelector(
      "label.switch > input",
    );
  ...
  chrome.storage.sync.get(
    ["nativeVersion", "nativeVersionStatus", "isNativeConnected"],
    function (result) {
...
function handleExclusionsChanges() {
  ...
  let returnFromExclusionsLabel = document.getElementById(
    "returnFromExclusionsLabel",
  );
  ...
  let videoExclusionsInputForm = document.getElementById(
    "videoExclusionsInput",
  );
  let videoExclusionsInputField =
    videoExclusionsInputForm.querySelector("input");
  videoExclusionsInputField.addEventListener("input", function (temp) {
    if (
      videoExclusionsInputField.value &&
      !isAlreadyInList(videoExclusionsInputField.value, "videoExclusionsList")
    ) {
      root.style.setProperty(
        "--videoExclusionsInputButtonBackgroundColor",
        "rgb(52, 199, 89)",
      );
      root.style.setProperty(
        "--videoExclusionsInputButtonTextColor",
        "rgb(255, 255, 255)",
      );
      root.style.setProperty(
        "--videoExclusionsInputButtonBackgroundColorHover",
        "rgb(48, 180, 80)",
      );
    } else {
      root.style.setProperty(
  ...
  let videoExclusionsInputButton =
    videoExclusionsInputForm.querySelector("span");
  ...
  videoExclusionsList.addEventListener("click", function (element) {
    if (element.target.className == "removeIeButton") {
      let exclusion = element.target.id.substring(
        VIDEO_EXCLUSION_REMOVE_KEY.length,
      );
      chrome.storage.sync.get("videoExclusionsList", function (result) {
  ...
  let keywordExclusionsInputForm = document.getElementById(
    "keywordExclusionsInput",
  );
  let keywordExclusionsInputField =
    keywordExclusionsInputForm.querySelector("input");
  keywordExclusionsInputField.addEventListener("input", function (temp) {
    if (
      ...
      root.style.setProperty(
        "--keywordExclusionsInputButtonBackgroundColor",
        "rgb(52, 199, 89)",
      );
      root.style.setProperty(
        "--keywordExclusionsInputButtonTextColor",
        "rgb(255, 255, 255)",
      );
      root.style.setProperty(
        "--keywordExclusionsInputButtonBackgroundColorHover",
        "rgb(48, 180, 80)",
      );
    } else {
      root.style.setProperty(
  ...
  let keywordExclusionsInputButton =
    keywordExclusionsInputForm.querySelector("span");
  ...
  keywordExclusionsList.addEventListener("click", function (element) {
    if (element.target.className == "removeIeButton") {
      let exclusion = element.target.id.substring(
        KEYWORD_EXCLUSION_REMOVE_KEY.length,
      );
      chrome.storage.sync.get("keywordExclusionsList", function (result) {
...
function handleInclusionsChanges() {
  ...
  let returnFromInclusionsLabel = document.getElementById(
    "returnFromInclusionsLabel",
  );
  ...
  let root = document.querySelector(":root");
  let videoInclusionsInputForm = document.getElementById(
    "videoInclusionsInput",
  );
  let videoInclusionsInputField =
    videoInclusionsInputForm.querySelector("input");
  videoInclusionsInputField.addEventListener("input", function (temp) {
    if (
      videoInclusionsInputField.value &&
      !isAlreadyInList(videoInclusionsInputField.value, "videoInclusionsList")
    ) {
      root.style.setProperty(
        "--videoInclusionsInputButtonBackgroundColor",
        "rgb(35, 155, 77)",
      );
      root.style.setProperty(
        "--videoInclusionsInputButtonTextColor",
        "rgb(210, 210, 210)",
      );
      root.style.setProperty(
        "--videoInclusionsInputButtonBackgroundColorHover",
        "rgb(35, 180, 77)",
      );
    } else {
      root.style.setProperty(
  ...
  let videoInclusionsInputButton =
    videoInclusionsInputForm.querySelector("span");
  ...
  videoInclusionsList.addEventListener("click", function (element) {
    if (element.target.className == "removeIeButton") {
      let inclusion = element.target.id.substring(
        VIDEO_INCLUSION_REMOVE_KEY.length,
      );
      chrome.storage.sync.get("videoInclusionsList", function (result) {
  ...
  let keywordInclusionsInputForm = document.getElementById(
    "keywordInclusionsInput",
  );
  let keywordInclusionsInputField =
    keywordInclusionsInputForm.querySelector("input");
  keywordInclusionsInputField.addEventListener("input", function (temp) {
    if (
      ...
      root.style.setProperty(
        "--keywordInclusionsInputButtonBackgroundColor",
        "rgb(35, 155, 77)",
      );
      root.style.setProperty(
        "--keywordInclusionsInputButtonTextColor",
        "rgb(210, 210, 210)",
      );
      root.style.setProperty(
        "--keywordInclusionsInputButtonBackgroundColorHover",
        "rgb(35, 180, 77)",
      );
    } else {
      root.style.setProperty(
  ...
  let keywordInclusionsInputButton =
    keywordInclusionsInputForm.querySelector("span");
  ...
  keywordInclusionsList.addEventListener("click", function (element) {
    if (element.target.className == "removeIeButton") {
      let inclusion = element.target.id.substring(
        KEYWORD_INCLUSION_REMOVE_KEY.length,
      );
      chrome.storage.sync.get("keywordInclusionsList", function (result) {
...
function handleEditPresenceChanges() {
  ...
  editPresenceLabel.onclick = function () {
    if (
      document
        .getElementById("editPresenceLabel")
        .classList.contains("flashOrange")
    ) {
      saveStorageKey("flashEditPresence", false);
      document
        .getElementById("editPresenceLabel")
        .classList.remove("flashOrange");
    }
    ytdpSettingsOutside.style.display = "none";
  ...
  let returnFromEditPresenceLabel = document.getElementById(
    "returnFromEditPresenceLabel",
  );
  ...
  enableChannelButtonLabel
    ...
    .addEventListener("change", function () {
      chrome.storage.sync.get("enableChannelButton", function (result) {
        let status =
          enableChannelButtonLabel.querySelector("span.switchStatus");
        handleSwitchStatusAndStorage(
          status.innerHTML == "OFF",
          status,
          "enableChannelButton",
...
window.onload = function () {
  getCurrentTab()
    .then((tab) => {
      ...
      document
        .getElementById("extensions-link")
